...
async function renderKlanken(app) {
  ...
  app.querySelectorAll('.klank-word').forEach(b => b.addEventListener('click', () => { speak(b.dataset.say, b); b.classList.add('said'); setTimeout(() => b.classList.remove('said'), 400); }));
}
...
async function renderBelgie(app) {
  ...
  function showSec(i) {
    ...
    body.querySelectorAll('.bspeak').forEach(b => b.addEventListener('click', () => speak(b.dataset.say, b)));
    bindVspeak(body);
